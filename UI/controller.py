...
class Controller:
    ...
    def handleCercaRaggiungibili(self,e):
        if self._fermataPartenza is None:
            self._view.lst_result.controls.clear()
            self._view.lst_result.controls.append(ft.Text("Attenzione non è stara fatta una scelta di stazione di partenza!", color="red"))
            self._view.update_page()
            return
            #sesuper l'if l'utente ha scelto tutto correttamente
